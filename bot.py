...
# ---------------------------------------------------------
# HELP PERSONNALISÉ (Plus joli)
# ---------------------------------------------------------
class MyHelp(commands.HelpCommand):
    async def send_bot_help(self, mapping):
        embed = discord.Embed(title="📜 Liste des commandes", color=discord.Color.blue())
        for cog, commands_list in mapping.items():
            command_signatures = [f"`?{c.name}`" for c in commands_list if not c.hidden]
            if command_signatures:
                cog_name = cog.qualified_name if cog else "Général"
                embed.add_field(name=cog_name, value=" ".join(command_signatures), inline=False)
        embed.set_footer(text="Fais ?help <commande> pour plus de détails")
        await self.get_destination().send(embed=embed)

    async def send_command_help(self, command):
        embed = discord.Embed(title=f"Aide pour `{command.name}`", description=command.help, color=discord.Color.green())
        await self.get_destination().send(embed=embed)
...
async def ban(ctx, member: discord.Member, *, reason: str = "Aucune raison fournie"):
    ...
    if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
        return await ctx.send("❌ Tu ne peux pas bannir ce membre (rôle égal ou supérieur au tien).")

    try:
        embed_mp = discord.Embed(title=f"🔨 Banni de {ctx.guild.name}", color=discord.Color.red())
...
@bot.command(name="unban", help="Débannir un membre. Utilisation : ?unban Pseudo#0000 ou ID")
@commands.has_permissions(ban_members=True)
@commands.bot_has_permissions(ban_members=True)
async def unban(ctx, *, user_input: str):
    ...
    for ban_entry in banned_users:
        user = ban_entry.user
        if str(user.id) == user_input or str(user) == user_input or user.name == user_input:
            user_to_unban = user
            break
...
@bot.command(name="kick", help="Expulser un membre. Utilisation : ?kick @membre [raison]")
...
async def kick(ctx, member: discord.Member, *, reason: str = "Aucune raison fournie"):
    ...
    if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
        return await ctx.send("❌ Tu ne peux pas expulser ce membre (rôle égal ou supérieur au tien).")

    try:
        embed_mp = discord.Embed(title=f"👢 Expulsé de {ctx.guild.name}", color=discord.Color.orange())
        embed_mp.add_field(name="Raison", value=reason, inline=False)
        embed_mp.add_field(name="Modérateur", value=ctx.author.mention, inline=False)
        await member.send(embed=embed_mp)
    except discord.Forbidden:
...
@bot.command(name="mute", help="Mute un membre (timeout, en minutes). Utilisation : ?mute @membre 10 [raison]")
...
async def mute(ctx, member: discord.Member, minutes: int, *, reason: str = "Aucune raison fournie"):
    ...
    if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
        return await ctx.send("❌ Tu ne peux pas mute ce membre (rôle égal ou supérieur au tien).")
    if minutes <= 0 or minutes > 40320:
        return await ctx.send("⚠️ La durée doit être comprise entre 1 minute et 28 jours (40320 minutes).")

    duration = timedelta(minutes=minutes)

    try:
        embed_mp = discord.Embed(title=f"🔇 Mute dans {ctx.guild.name}", color=discord.Color.gold())
...
@bot.command(name="unmute", help="Retirer le mute d'un membre. Utilisation : ?unmute @membre")
@commands.has_permissions(moderate_members=True)
@commands.bot_has_permissions(moderate_members=True)
...
@bot.command(name="clear", help="Supprimer un nombre de messages. Utilisation : ?clear 10")
...
async def clear(ctx, nombre: int):
    ...
    if nombre > 100:
        return await ctx.send("⚠️ Tu ne peux pas supprimer plus de 100 messages à la fois.")
...
@bot.command(name="say", help="Fait répéter un message par le bot. Utilisation : ?say <message>")
...
@bot.command(name="annonce", help="Créer une annonce sous forme d'embed. Utilisation : ?annonce <texte>")
@commands.has_permissions(administrator=True)
...
@bot.command(name="8ball", help="Pose une question à la boule magique. Utilisation : ?8ball <question>")
async def eight_ball(ctx, *, question: str):
    reponses = [
        "Oui, sans aucun doute.", "C'est certain.", "Probablement.",
        "Demande à nouveau plus tard.", "Je ne peux pas le dire pour l'instant.",
        "Non, ne compte pas dessus.", "Mes sources disent non.",
        "C'est très douteux.", "Concentre-toi et redemande."
    ]
...
@bot.command(name="avatar", help="Affiche l'avatar d'un membre. Utilisation : ?avatar [@membre]")
...
@bot.command(name="slap", help="Mets une baffe à quelqu'un. Utilisation : ?slap @membre")
async def slap(ctx, member: discord.Member):
    await ctx.send(f"👋 {ctx.author.mention} met une énorme claque à {member.mention} !")
